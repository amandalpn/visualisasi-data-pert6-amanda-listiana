...
import { ClipboardCopy, Check, Filter as FilterIcon } from 'lucide-react';
...
const InsightsPage = () => {
  ...
  const insights = useMemo(() => {
    ...
    return [
      ...
      { title: 'Outcome Modul', body: [craftModuleInsight(moduleOutcome)] },
      { title: 'Aktivitas VLE', body: [craftActivityInsight(activity)] },
      { title: 'Distribusi Nilai', body: [craftHistogramInsight(grade)] },
      { title: 'Demografi', body: [craftDemographicInsight(demo)] },
      { title: 'Sebaran Wilayah', body: [buildRegionInsight(region)] },
      { title: 'Mahasiswa Berisiko', body: [craftScatterInsight(students)] },
    ];
  }, [data, filters]);
      ...
      >
        <header className="space-y-4 rounded-3xl bg-gradient-to-r from-rose-500/20 via-amber-500/10 to-violet-500/20 p-6 text-slate-900 shadow-glass backdrop-blur-2xl dark:text-white">
          ...
          <p className="max-w-3xl text-sm leading-6 text-slate-700 dark:text-slate-200">
            Laporan ringkas berisi temuan utama dari visualisasi yang sedang aktif. Gunakan tombol
            salin untuk menempelkan insight ini ke laporan tugas atau bahan koordinasi manajemen
            kampus.
          </p>
        </header>
        ...
        <div className="grid gap-6 lg:grid-cols-[var(--fsb-w,320px),1fr]">
          ...
          <CardContent className="prose prose-slate max-w-none text-justify dark:prose-invert">
